...
const Machines = () => {
  ...
  const handleGeneratePlist = async () => {
    try {
      const response = await apiClient.post(
        `/api/machines/${selectedMachine.machine_id}/plist`,
        plistData,
        { responseType: 'blob' }
      );
      ...
      link.href = url;
      link.setAttribute('download', `${selectedMachine.machine_id}.plist`);
      document.body.appendChild(link);
      link.click();
      link.remove();

      setSuccess('Plist downloaded successfully!');
      setOpenPlist(false);
      setTimeout(() => setSuccess(''), 3000);
    } catch (error) {
  ...
  return (
    <Box>
      ...
      <Dialog open={openPlist} onClose={() => setOpenPlist(false)}>
        <DialogTitle>Generate Plist Configuration</DialogTitle>
        <DialogContent>
          <TextField
            fullWidth
            select
            label="Client Mode"
            value={plistData.client_mode}
            onChange={(e) => setPlistData({ ...plistData, client_mode: e.target.value })}
            margin="normal"
          >
          ...
          <TextField
            ...
            value={plistData.upload_interval}
